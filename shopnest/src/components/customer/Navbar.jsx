...
export default function Navbar({ user, onLogout }) {
    ...
    // Fetch all products once for fuzzy search
    useEffect(() => {
            ...
            <div className="container mx-auto px-4 py-3">
                        ...
                        {user ? (
                            <div className="relative group hidden md:block">
                                ...
                                <div className="absolute right-0 hidden group-hover:block bg-white shadow-xl rounded-md mt-2 py-1 w-48 z-10 transition-all duration-300 ease-in-out transform scale-95 opacity-0 group-hover:scale-100 group-hover:opacity-100">
                                    <Link to="/customer/profile" className="block px-4 py-2 text-gray-700 hover:bg-gray-100">My Profile</Link>
                                    <Link to="/customer/orders" className="block px-4 py-2 text-gray-700 hover:bg-gray-100">My Orders</Link>
                                    <button onClick={onLogout} className="block w-full text-left px-4 py-2 text-gray-700 hover:bg-gray-100">Logout</button>
                                </div>
                            </div>
                        ) : (
                            <Link to="/customer/login" className="hidden md:block text-gray-700 hover:text-blue-600 font-medium p-2 rounded-md hover:bg-gray-100">Login</Link>
                        )}
                ...
                <div className={`md:hidden ${isMobileMenuOpen ? 'block' : 'hidden'} bg-gray-50 border-t border-gray-200 py-4`}>
                    <div className="flex flex-col items-center space-y-4">
                        <Link to="/" className="text-gray-700 hover:text-blue-600" onClick={toggleMobileMenu}>Home</Link>
                        <Link to="/customer/categories" className="text-gray-700 hover:text-blue-600" onClick={toggleMobileMenu}>Categories</Link>
                        {user ? (
                            <>
                                <Link to="/customer/profile" className="text-gray-700 hover:text-blue-600" onClick={toggleMobileMenu}>My Profile</Link>
                                <Link to="/customer/orders" className="text-gray-700 hover:text-blue-600" onClick={toggleMobileMenu}>My Orders</Link>
                                <button onClick={() => { onLogout(); toggleMobileMenu(); }} className="text-gray-700 hover:text-blue-600">Logout</button>
                            </>
                        ) : (
                            <Link to="/customer/login" className="text-gray-700 hover:text-blue-600" onClick={toggleMobileMenu}>Login</Link>
                        )}
                    </div>
                </div>
            </div>
